...
def feature2csv():
    ...
    la.to_csv('./csv/train/labels.csv', index=False, header=False)

    print('訓練資料', len(data), '筆')

##############################################################################################
    data = pd.read_csv('D:/DATASET/UrbanSound8K/test.csv')
    print(data.head())  # To see the dataset
    ...
    for i in tqdm(range(len(data))):
        f_name = 'D:/DATASET/UrbanSound8K/Test/'+str(data.ID[i])+'.wav'
        ID.append(data.ID[i])
        X, s_rate = librosa.load(f_name, res_type='kaiser_fast')
        # mfccs
        mf = np.mean(librosa.feature.mfcc(y=X, sr=s_rate, hop_length= 1024,n_mfcc=22).T, axis=0)
    ...
    mfcc = pd.DataFrame(mfc)
    mfcc.to_csv('./csv/test/mfcc.csv', index=False, header=False)
    # chromagram = pd.DataFrame(chro)
    # chromagram.to_csv('./csv/chromagram.csv', index=False, header=False)
    # mel = pd.DataFrame(me)
    # mel.to_csv('./csv/mel.csv', index=False, header=False)
